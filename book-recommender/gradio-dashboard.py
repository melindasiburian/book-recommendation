...
def build_theme():
    ...
    if themes and hasattr(themes, "Glass"):
        return themes.Glass()
    if themes and hasattr(themes, "Default"):
...
def build_embeddings() -> OpenAIEmbeddings:
    ...
    if not openrouter_api_key:
        raise RuntimeError(
            "OPENROUTER_API_KEY is not set. Add your OpenRouter key to .env as "
            "OPENROUTER_API_KEY=..."
        )
...
db_books = Chroma.from_documents(documents, build_embeddings())
...
with gr.Blocks(theme=build_theme()) as dashboard:
